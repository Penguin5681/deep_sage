...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  final TextEditingController openAiApiInputController =
      TextEditingController();
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (image != null) {
          ...
          var request = http.MultipartRequest(
            'POST',
            Uri.parse(
              '${dotenv.env['DEV_BASE_URL']!}/api/upload-profile-photo',
            ),
          );

          request.fields['user_id'] = userId;
          request.files.add(
            await http.MultipartFile.fromPath('file', uploadImagePath),
          );
  ...
  Future<Image> loadProfileImageFromHive() async {
    ...
    if (userId.isNotEmpty) {
      try {
        final awsImageUrl =
            '${dotenv.env['DEV_BASE_URL']!}/api/aws/s3/get-profile-photo/$userId';
                                    ...
                                    child: GestureDetector(
                                      onTap: () async {
                                        final result = await _cacheService
                                            .showCacheClearDialog(context);
                                        if (!context.mounted) return;

                                        final shouldRestart =
                                            result['shouldRestart'] ?? false;
                                        final didClearCache =
                                            result['didClearCache'] ?? false;
